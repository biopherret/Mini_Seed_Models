...
def _get_L_mat(parr, N_nucl, Nb, n_max, num_steps, h, L0_mat, L_mat):
    ...
    for t_step in range(1,num_steps):
        current_L = L_mat[0]
        for n in range(n_max + 2):
            pkp = mdf.part_kp(current_L, n, Nb, n_max)
            pkp0 = mdf.part_kp0(current_L, n, N_nucl, Nb, n_max)
            pkd = mdf.part_kd(current_L, n, N_nucl, Nb, n_max)
            pkb = mdf.part_kbreak(current_L, n, n_max)
            L_mat[1,n] = parr[0] * pkp + parr[1] * pkp0 + parr[2] * pkd + parr[3] * pkb
        for n in range(n_max + 2):
            L_mat[0,n] = mdf.vec_add(current_L, mdf.scal_x_vec(h, L_mat[1]))[n]
